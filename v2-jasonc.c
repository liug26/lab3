...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//initialize a lock here to lock each list head
		ecode = pthread_mutex_init(&entry->mutex1, NULL);
		if (ecode != 0) {
			exit(ecode);
		}
	}
                             ...
                             uint32_t value)
{
	...
	//lock
	int ecode = pthread_mutex_lock(&hash_table_entry->mutex1);
	if (ecode != 0) {
		exit(ecode);
	}
	if (list_entry != NULL) {
		...
		//unlock
		ecode = pthread_mutex_unlock(&hash_table_entry->mutex1);
		if (ecode != 0) {
			exit(ecode);
		}
		return;
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//destroy each lock
		int ecode = pthread_mutex_destroy(&entry->mutex1);
		if (ecode != 0) {
			exit(ecode);
		}
	}
