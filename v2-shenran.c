...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int init = pthread_mutex_init(&entry->foo_lock, NULL);
		if(init != 0){
			exit(init);
		}
                             ...
                             uint32_t value)
{
	...
	int lock_status = pthread_mutex_lock(&hash_table_entry->foo_lock);
	...
	if (list_entry != NULL) {
		...
		int unlock_other = pthread_mutex_unlock(&hash_table_entry->foo_lock);
		if (unlock_other != 0){
			exit(unlock_other);
		}
		return;
	...
	int unlock_status = pthread_mutex_unlock(&hash_table_entry->foo_lock);
	if(unlock_status != 0){
		exit(unlock_status);
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int destroy = pthread_mutex_destroy(&entry->foo_lock);
		if(destroy != 0){
			exit(destroy);
		}
	}
